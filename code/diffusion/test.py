...
from networks.GraphTransformerModel import *
...
def test(args):
    n_layers = 5
    input_dims = {"X": 3, "E": 10, "y": 4}
    hidden_mlp_dims = {'X': 256, 'E': 128, 'y': 128}
    hidden_dims = {'dx': 256, 'de': 64, 'dy': 64, 'n_head': 8, 'dim_ffX': 256, 'dim_ffE': 128, 'dim_ffy': 128}
    output_dims = {"X": 3, "E": 10, "y": 4}
    act_fn_in = nn.ReLU()
    act_fn_out = nn.ReLU()

    transformer = GraphTransformer(n_layers, input_dims, hidden_mlp_dims, hidden_dims, output_dims, act_fn_in, act_fn_out)

    n_nodes = 135
    node_mask = torch.ones(4, n_nodes)
    out = transformer(torch.randn(4, 135, 3), torch.randn(4, 135, 135, 10), torch.randn(4, 4), node_mask)
...
def test2(args):
    ...
    diffusion = DiffusionWrapper(denoising_fn=model, diffusion_model=diffusion_model, lr=args.lr,
